...
output = {
    ...
    "monitors": [],
    "extensions": [],
    "meta": {"semver": "3.4.0", "vm": "2.3.0", "agent": "Thanks for using CatSh ❤️"},
}
var = {}
lists = {}
totalblocks = 0
    ...
    if block[2] == blockname:
        ...
        for i in range(len(keys)):
            try:
                ...
                warnings.append(
                    f"Failed to read parameter {i+1} for block {totalblocks} ({block[2]})"
                )
        for i in range(len(field)):
            fields[field[i]] = [block[3 + i + len(keys)].strip('"'), None]
        output["targets"][target]["blocks"][f"""{blocksbyid[f'blk{totalblocks}']}"""][
            "inputs"
        ] = outputs
    ...
    try:
        ...
        if block[0] != "define" and block[0] != "import":
            ...
            for key in a:
                ...
                vn = c[1:-1]
                if c[0] == "<" and c[-1] == ">":
                    output["targets"][target]["blocks"][f"{blocks[orgblock]}"][
                        "inputs"
                    ][key] = [3, [12, vn, varsbyid[vn]], [4, "0"]]
...
def blockEval(block, target):
    ...
    blocks[orgblock] = "".join(
        random.choice(
            "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ[]-!*{}"
        )
        for _ in range(32 - len(str(totalblocks + 1)))
    ) + str(totalblocks + 1)
    ...
    if block[0] == "define":
        ...
        if block[1] == "var":
            ...
            output["monitors"].append(
                {
                    "id": f"var{totalvariables}",
                    "mode": "default",
                    "opcode": "data_variable",
                    "params": {"VARIABLE": block[2]},
                    "value": block[4],
                    "spritename": None,
                    "width": 0,
                    "height": 0,
            ...
            output["targets"][0]["variables"][f"""var{totalvariables}"""] = [
                block[2],
                block[4],
            ]
    elif block[0] == "import":
        ...
        if block[1] == "png":
            output["targets"][target]["costumes"].append(
                {
                    "name": block[2].split(".")[0],
                    "dataFormat": "png",
                    "assetId": assetId,
                    "md5ext": assetId + ".png",
                    "rotationCenterX": 240,
        ...
        if block[1] == "wav":
            ...
            output["targets"][target]["sounds"].append(
                {
                    "name": block[2].split(".")[0],
                    "dataFormat": "wav",
                    "assetId": assetId,
        ...
        output["targets"][target]["blocks"][f"""{blocks[orgblock]}"""] = {
            "opcode": block[2],
            "next": f"blk{block[1]}" if block[1] != "-1" else None,
            "parent": f"blk{block[0]}" if block[0] != "-1" else None,
            "fields": {},
            "inputs": {},
            "shadow": False,
            "topLevel": True,
...
if sys.argv[1] == "convert":
    ...
    for f in range(len(inputfile)):
        ...
        if i == "<":
            ...
            if i == ">":
                print(
                    f"\x1b[33mvariable\x1b[0m reference for \x1b[33m<{varname}>\x1b[0m inside block #{totalblocks}"
                )
    ...
    if len(warnings) > 0:
        print(
            f"\x1b[31m🚫 {len(warnings)} Warning{'' if len(warnings) == 1 else ''} Occured:"
        )
    ...
    print("\x1b[0m", end="")
    print("📦 Converted in " + str((end - start)))
